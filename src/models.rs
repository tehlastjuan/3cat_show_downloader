...
impl Episode {
    pub fn filename(&self, extension: &str) -> String {
        ...
        let tmp_title = collapsed.trim_matches('_').to_string();

        let reg = Regex::new(r"(capitol_)?([0-9]+)?(_)?(.*)").unwrap();
        let title = reg.replace(&tmp_title, "$4".to_string()).to_string();
        ...
        if self.tv_show_name.to_lowercase().contains("ova") {
            format!("ova_{}_{}.{extension}", self.episode_number, title)
        } else {
            format!("{}_{}.{extension}", self.episode_number, title)
        }
    }
}
